...
3. **센서**
   - MPU6050 센서를 통해 X, Y, Z값을 읽고 Wi-Fi를 통해 서버로 전송
4. **서버**
...
이 프로젝트를 진행하면서 제일 많이 마주했던 문제는 서버와 클라이언트 간의 데이터 교환이였다.   
서버에 관련해서는 지식이 거의 없었기에 서칭을 하는 데에 많은 시간이 소요되었다.  
첫 시도는 무료 호스팅 서버였다. 이 때 FileZilla를 알게되었고 서버에 php파일을 이동시키는 것을 알았다.  
